/*  Compute nCr for each test case and return ans modulo 1e9+7;
*/
   ...
   ll compute_nCr(ll n, ll r){ //O(logn * logn)
     return (fact[n]%p * power(fact[n-r], p-2, p)%p * power(fact[r], p-2, p)%p)%p;
   }

   ll optimized_nCr(ll n, ll r){ // O(1) 
   ...
   int main(){
    ...
    while(T--){
      cin>>n>>r;
      cout<<compute_nCr(n, r)<<endl;  // O(T * logn * logn)
      cout<<optimized_nCr(n, r)<<endl; // O(T * 1)
    }
